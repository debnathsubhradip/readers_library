...
function showLoading() {
    ...
    if (displaylist) {
        displaylist.innerHTML = '<div class="text-center py-5"><div class="spinner-border text-primary" role="status"></div><p class="mt-3 text-muted">Loading resources...</p></div>';
    }
}
...
function loadresource(){
    ...
    .catch((err)=>{
        ...
        showError('Unable to load resources. Please check your internet connection and try again.');
        isLoading = false;
    });
}
loadresource();
...
function loadlist(){
    ...
    if (!responsearray || responsearray.length === 0) {
        ...
        displaylist.innerHTML = '<div class="alert alert-info"><h4>No Resources Found</h4><p>The resource repository appears to be empty.</p></div>';
        return;
    }
    let checklist=0;
    let displaylist=document.getElementById('displaylist');
    ...
    for(let i=0;i<responsearray.length;i++){
        ...
        let iconClass = fileExt === 'html' ? 'bi bi-filetype-html text-danger' : 'bi bi-file-earmark-text-fill text-primary';
        code+='<div class="col">';
        ...
        code+='<i class="bi bi-box-arrow-up-right me-2"></i>View Resource';
        code+='</a></div></div></div>';
    }
    code+="</div>";
    if(checklist===0){
        code='<div class="alert alert-info"><h4>No Resources Found</h4><p>No resources match your search criteria.</p></div>';
    }
...
function search(){
    ...
    code+="</div>";
    if(checklist===0){
        code='<div class="alert alert-warning"><h4>No Resources Found</h4><p>No resources match your search. Try a different keyword.</p></div>';
    }
    let para=document.createElement("DIV");
    para.id='booklist';
    para.innerHTML=code;
